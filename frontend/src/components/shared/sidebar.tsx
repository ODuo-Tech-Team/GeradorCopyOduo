...
const NAV = [
  ...
  { href: "/generate", label: "Gerar Copy", shortLabel: "Gerar", icon: Sparkles },
  { href: "/upload", label: "Upload", shortLabel: "Upload", icon: Upload },
...
export function Sidebar() {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden md:flex w-64 border-r border-zinc-800 bg-zinc-950 flex-col">
        <div className="p-6">
      ...
      {/* Mobile bottom nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-zinc-950 border-t border-zinc-800 flex justify-around items-center px-2 py-2 safe-area-bottom">
        {NAV.map(({ href, shortLabel, icon: Icon }) => {
          ...
          return (
            <Link
              ...
              href={href}
              className={cn(
                "flex flex-col items-center gap-1 px-3 py-1.5 rounded-lg text-xs transition-colors min-w-[60px]",
